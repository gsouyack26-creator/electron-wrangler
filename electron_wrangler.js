...
function renderLive(){ var box=$("#simextra"); if(!box)return; var v=_liveSrcVolts();
  ...
  if(!r.ok) h+=`<div class="hint" style="color:var(--warn)">`+esc(r.msg)+`</div>`;
    ...
    h+=`<ol style="padding-left:18px;margin:6px 0">`+r.path.map(function(p){ var c=p.comp; var loc=c.phys?` \u2014 <span style="color:var(--live)">on the panel photo</span>`:``; return `<li style="margin-bottom:6px;cursor:pointer" data-lc="`+esc(c.id)+`"><b>`+esc(c.label||compDef(c).name)+`</b>`+loc+`<br><span style="color:var(--dim);font-size:.85em">`+esc(_liveWhy(c,p.contact))+`</span></li>`; }).join("")+`</ol>`;
  }
