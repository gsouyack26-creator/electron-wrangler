...
function solve(){
  ...
  comps.forEach(c=>{ const d=compDef(c);
    ...
    if(d.load){ const ph=c.phases||(c.type==='motor'?3:1);
      if(c.type==='motor'&&ph===3){ c._on = ['U','V','W'].every(t=>_isHot(_key(c,t))); }
...
function render(){
  ...
  PANEL.wires.forEach(w=>{ const a=endPos(w.a),b=endPos(w.b); if(!a||!b)return;
    ...
    s+=`<path class="${cls.join(' ')}" data-wire="${w.id}" d="${wirePath(a,b)}"/>`;
  ...
  // components
  PANEL.components.forEach(c=>{ const d=compDef(c);
    const cls=['compbox']; if(sel===c||selSet.indexOf(c)>=0)cls.push('sel'); if(mode==='sim'&&anyEnerg(c))cls.push('energized'); if(c.fault)cls.push('faulted'); if(hoverData&&hoverData.comps.has(c.id))cls.push('hoverpath');
    let body=d.body(c);
...
function runDiag(c){ const r=diagnose(c); const box=$('#diag');
  ...
  const _wb=box.querySelector('#diag-walk'); if(_wb)_wb.onclick=()=>{ const steps=guidedWalk(c); $('#walkout').innerHTML=steps.map(t=>`<div class="suspect" style="cursor:default;border-color:var(--accent)">${esc(t)}</div>`).join(''); };
