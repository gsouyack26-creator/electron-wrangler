...
function render(){
  ...
  if(PANEL.backdrop){ const o=($('#opacity').value)/100;
    s+=`<image href="${PANEL.backdrop}" x="0" y="0" width="${PANEL.bw||1200}" height="${PANEL.bh||800}" opacity="${o}" preserveAspectRatio="xMidYMid meet"/>`; }
  ...
  PANEL.wires.forEach(w=>{ const a=endPos(w.a),b=endPos(w.b); if(!a||!b)return;
    ...
    let _pstyle=''; if(showPhase&&mode==='sim'&&w._phase&&w._phase!=='mix'&&PHASE_COLOR[w._phase]) _pstyle=` style="stroke:${PHASE_COLOR[w._phase]}"`;
  ...
  // components
  PANEL.components.forEach(c=>{ const d=compDef(c);
    const cls=['compbox']; if(sel===c||selSet.indexOf(c)>=0)cls.push('sel'); if(mode==='sim'&&anyEnerg(c))cls.push('energized'); if(c.fault)cls.push('faulted'); if(hoverData&&hoverData.comps.has(c.id))cls.push('hoverpath'); if(mode==='sim'&&c._singlePhase)cls.push('singlephase');
    let body=d.body(c);
    ...
    if(cbSafe&&mode==='sim'){ const g=c.fault||c.hiZ?'\u26a0':c._singlePhase?'1\u00d8':(anyEnerg(c)?'\u26a1':''); if(g)s+=`<text x="${d.w/2}" y="${d.h/2+3}" style="fill:#fff;font:bold 10px sans-serif;text-anchor:middle;paint-order:stroke;stroke:#000;stroke-width:3px">${g}</text>`; }
    s+=`</g>`; });
  // wire-in-progress
...
function renderSimInspector(){ const box=$('#inspector'); const c=sel;
  if(!c){ box.innerHTML=`<h3>Simulate</h3>
    ...
    $('#sim-meter').onclick=()=>{meterMode=!meterMode;probes=[];render();renderSimInspector();};
  ...
  if(d.sw){ const cur=c.state; const cls=(cur==='closed'||cur==='ok'||cur==='on')?'closed':(cur==='tripped'||cur==='blown')?'fault':'open';
    h+=`<button class="statebtn ${cls}" id="sim-toggle">State: ${cur.toUpperCase()} — click to change</button>`; }
...
function setMode(m){ mode=m; wireStart=null;tool=null;setTool(null); meterMode=false; probes=[]; hoverComp=null; hoverData=null; logMode=false; showVolts=false; pinMode=false; selSet=[]; marquee=null; netHi=null; if(quiz) quizEnd();
...
$('#fileJson').onchange=e=>{ const f=e.target.files[0]; if(!f)return; const r=new FileReader();
  r.onload=()=>{ try{ PANEL=validatePanel(JSON.parse(r.result)); uid=Date.now()%1e6; restoreUid(); sel=null;selWire=null; persist(); applyEnc();render(); renderInspector(); toast('Panel loaded'); }catch(err){toast('Bad file');} }; r.readAsText(f); e.target.value=''; };
...
function physInfo(c){ if(!c)return''; const nb=wiredNeighbors(c).map(n=>n.label||n.id);
  ...
  return '<div class="hint" style="margin:0 0 8px;padding:8px;border:1px solid var(--edge);border-radius:6px;background:var(--panel2)">'
    ...
    +'<span style="color:var(--dim)">Wired to:</span> '+(nb.join(', ')||'(nothing)')+'</div>'; }
...
function partsLink(c){ const url=(function(){try{return (localStorage.getItem('pt_parts_url')||'').trim();}catch(e){return '';}})();
  ...
  if(url){ window.open(url+(url.includes('?')?'&':'?')+'q='+encodeURIComponent(c.apn),'_blank'); }
...
function openSnapshots(){ const snaps=PANEL._snaps||[];
  ...
  document.querySelectorAll('[data-r]').forEach(el=>el.onclick=()=>{ const s=snaps[+el.dataset.r];
  ...
  document.querySelectorAll('[data-d]').forEach(el=>el.onclick=()=>{ const s=snaps[+el.dataset.d]; const d=diffPanels(s.data);
...
/* ---------- 7: project / tabs ---------- */
function projPersist(){ if(PROJECT){ try{ localStorage.setItem('acy1_project',JSON.stringify(PROJECT)); }catch(e){} } }
function projRestore(){ try{ const s=localStorage.getItem('acy1_project'); if(s){ PROJECT=JSON.parse(s); return true; } }catch(e){} return false; }
...
function projSwitch(i){ if(!PROJECT||i<0||i>=PROJECT.panels.length)return; projSaveActive(); PROJECT.active=i;
  ...
  persist(); applyEnc(); if(typeof applyView==='function')applyView(); render(); renderInspector(); renderTabs(); }
...
function openPM(){ PANEL.pm=PANEL.pm||PM_DEFAULT.map(t=>({text:t,done:false}));
  ...
  document.querySelectorAll('[data-i]').forEach(el=>el.onchange=()=>{ PANEL.pm[+el.dataset.i].done=el.checked; persist(); });
...
function openScenarios(){ let best={}; SCEN.forEach(s=>{try{best[s.id]=localStorage.getItem('pt_scen_'+s.id);}catch(e){}});
  openModal('Training scenarios', SCEN.map(s=>'<div class="suspect" style="cursor:pointer" data-s="'+s.id+'"><b>'+esc(s.name)+'</b>'+(best[s.id]?' <span style="color:var(--ok)">\u00b7 best '+best[s.id]+'s</span>':'')+'<br><span class="hint">'+esc(s.prompt)+'</span></div>').join(''));
  document.querySelectorAll('[data-s]').forEach(el=>el.onclick=()=>startScenario(el.dataset.s)); }

/* ---------- 7: colorblind-safe ---------- */
function applyCb(){ const st=document.getElementById('stage'); if(st)st.classList.toggle('cbsafe',cbSafe);
  const b=$('#btnCb'); if(b)b.classList.toggle('on',cbSafe);
  try{localStorage.setItem('pt_cb',cbSafe?'1':'0');}catch(e){} }

/* ---------- 9: touch pinch-zoom ---------- */
function _rect(){ return svg.getBoundingClientRect(); }
function initPinch(){
  svg.addEventListener('pointerdown',e=>{ _pointers[e.pointerId]={x:e.clientX,y:e.clientY}; },true);
  svg.addEventListener('pointermove',e=>{ if(_pointers[e.pointerId])_pointers[e.pointerId]={x:e.clientX,y:e.clientY};
    const ids=Object.keys(_pointers); if(ids.length>=2){ panning=null; drag=null;
      ...
      if(_pinch){ const nk=Math.max(.25,Math.min(4,_pinch.k*(dist/_pinch.d)));
        view.x=mx-(mx-_pinch.vx)*(nk/_pinch.k); view.y=my-(my-_pinch.vy)*(nk/_pinch.k); view.k=nk; render(); }
      else _pinch={d:dist,k:view.k,vx:view.x,vy:view.y}; } },true);
  const end=e=>{ delete _pointers[e.pointerId]; if(Object.keys(_pointers).length<2)_pinch=null; };
...
function saveUserLib(o){ try{ localStorage.setItem('pt_userlib',JSON.stringify(o)); }catch(e){} }
...
function openLibraryManager(){ const lib=window.PANEL_LIBRARY||{}, mine=userLib();
  ...
  $('#lib-files').onchange=e=>{ const fs=[...e.target.files]; const o=userLib(); let n=0; let pend=fs.length;
    fs.forEach(f=>{ const r=new FileReader(); r.onload=()=>{ try{ const p=JSON.parse(r.result); const nm=p.name||f.name.replace(/\.panel\.json$|\.json$/,''); o[nm]=p; n++; }catch(err){} if(--pend===0){ saveUserLib(o); toast(n+' panel(s) imported'); openLibraryManager(); } }; r.readAsText(f); }); e.target.value=''; };
