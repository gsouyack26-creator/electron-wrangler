...
if os.path.exists(layout_path):
    ...
    layout_js = (
        "window.PANEL_LAYOUT="
        + json.dumps(layout, separators=(",", ":")).replace("</", "<\\/")
        + ";"
    )
...
if out == html:
    raise SystemExit(
        'ERROR: <script src="electron_wrangler.js"> anchor not found in template'
    )
...
if os.path.exists(_mpath) and '<link rel="manifest" href="manifest.json">' in out:
    with open(_mpath, encoding="utf-8") as _f:
        _mraw = _f.read()
    _durl = "data:application/manifest+json;base64," + _b64.b64encode(
        _mraw.encode("utf-8")
    ).decode("ascii")
    out = out.replace(
        '<link rel="manifest" href="manifest.json">',
        '<link rel="manifest" href="' + _durl + '">',
        1,
    )
