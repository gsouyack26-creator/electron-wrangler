...
function render(){
  ...
  PANEL.wires.forEach(w=>{ const a=endPos(w.a),b=endPos(w.b); if(!a||!b)return;
    ...
    if(showVolts&&mode==='sim'&&w._v!=null) s+=`<text class="voltlbl" x="${mx}" y="${(a.y+b.y)/2+9}">${w._v}V</text>`; });
  // components
  PANEL.components.forEach(c=>{ const d=compDef(c);
...
svg.addEventListener('pointerdown',ev=>{
  ...
  if(linkEl){ loadLibPanel(linkEl.dataset.link); return; }
...
svg.addEventListener('pointermove',ev=>{
  ...
  if(drag){ const p=toWorld(ev); const nx=Math.round(p.x-drag.dx), ny=Math.round(p.y-drag.dy);
...
/* ---------- palette / modes ---------- */
const PAL=['source','disc','breaker','fuse','contactor','overload','relay','vfd','motor','light','estop','pbNO','pbNC','selector','sensor','plcIn','plcOut','term'];
