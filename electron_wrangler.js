...
$('#btnAI').onclick=()=>{ const msg=`AI EXTRACT (hybrid workflow)\n\n`+
  ...
  `2. Send that same image to Aki in chat and say: "extract this panel to Electron Wrangler JSON".\n`+
...
var FIXMAP={
  ...
  'open control wire':{why:'There is an open in the control string feeding the coil, so the coil never energizes and the starter will not seal in.',fix:'Ring out the control string from the source to the coil. Find and re-land the open conductor / connection.'},
  'broken conductor':{why:'A conductor between the field device and the input card is broken, so the channel reads open even though the device itself is good.',fix:'Ring out the run end-to-end. Find the break (often at a terminal or a flex point) and repair/re-pull the conductor.'}
};
...
function openFlashcards(){ var deck=_flashDeck(); var fs=_flashState();
  ...
  function render(){ if(i>=queue.length){ openModal('\ud83c\udccf Flashcards \u2014 done', '<div class="hint" style="padding:14px;text-align:center">Session complete \u2014 <b>'+got+'</b> of '+queue.length+' marked <b>Got it</b>.<br>Mastered cards earn '+AET.XP.flash_graduate+' XP each.</div><div style="display:flex;gap:8px;margin-top:10px"><button class="tbtn" id="fc-more" style="flex:1">\ud83c\udccf More cards</button><button class="tbtn" id="fc-done" style="flex:1">\ud83c\udf93 Progression</button></div>');
    ...
    var c=queue[i];
    var card='<div style="text-align:center;margin-bottom:6px" class="hint">Card '+(i+1)+' of '+queue.length+' \u00b7 box '+c.box+'/5 \u00b7 '+c.tag+'</div>'
